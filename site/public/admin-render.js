...
export function resetProductSales(context) {
  ...
  if (context.elements.productSalesMetric) context.elements.productSalesMetric.value = "revenue";
  if (context.elements.productSalesMetricHeading) context.elements.productSalesMetricHeading.textContent = "Τζίρος";
  if (context.elements.productSalesSecondaryMetricHeading) context.elements.productSalesSecondaryMetricHeading.textContent = "Τεμάχια";
  if (context.elements.productSalesPagination) context.elements.productSalesPagination.hidden = true;
  if (context.elements.productSalesPageInfo) context.elements.productSalesPageInfo.textContent = "Σελίδα 1 από 1";
  if (context.elements.productSalesPrevBtn) context.elements.productSalesPrevBtn.disabled = true;
  if (context.elements.productSalesNextBtn) context.elements.productSalesNextBtn.disabled = true;
...
export function resetSearchResults(context) {
  ...
  if (context.elements.searchResultsPanel) context.elements.searchResultsPanel.hidden = true;
...
export function resetBranchSelector(context) {
  ...
  if (context.elements.branchSelectorPanel) context.elements.branchSelectorPanel.hidden = true;
...
export function renderFilteredBranchOptions(context, branches, selectedBranchCode = "") {
...
export function renderBranchSelector(context, customerCode, branches = [], selectedBranchCode = "") {
  context.state.currentCustomerCode = customerCode || null;
  context.state.currentBranchCode = selectedBranchCode || "";
  const isStatsLoading = context.elements.statsPanel?.getAttribute("aria-busy") === "true";
  const items = Array.isArray(branches) ? branches : [];
...
export function renderSearchResults(context, items, filters = {}) {
  ...
  if (context.elements.searchResultsPanel) context.elements.searchResultsPanel.hidden = false;
  const isStatsLoading = context.elements.statsPanel?.getAttribute("aria-busy") === "true";
  const activeFilters = Object.values(filters || {}).filter(Boolean).join(" | ");

  context.elements.searchResultsBody.innerHTML = context.state.currentSearchResults.length
    ? context.state.currentSearchResults
        .map((item) => {
          return `
            <tr>
...
export function renderSelectedOrderDetails(context) {
  const selectedOrder = context.findDetailedOrder(context.state.selectedOrderId);
  if (!selectedOrder) {
...
export function resetMonthlySales(context) {
  ...
  if (context.elements.monthlyYearOneHeading) context.elements.monthlyYearOneHeading.textContent = String(currentYear - 2);
  if (context.elements.monthlyYearTwoHeading) context.elements.monthlyYearTwoHeading.textContent = String(currentYear - 1);
  if (context.elements.monthlyYearThreeHeading) context.elements.monthlyYearThreeHeading.textContent = String(currentYear);
...
export function renderMonthlySales(context, monthlySales) {
  ...
  const displaySeries = [...yearlySeries].sort((a, b) => Number(a.year || 0) - Number(b.year || 0)).slice(-3);
  const rows = [];
  ...
  for (let month = 1; month <= 12; month += 1) {
    ...
    const revenueCells = revenues
      .map((revenue) => `<td class="admin-table-number${numberStateClass(revenue)}">${escapeHtml(formatMoney(revenue))}</td>`)
      .join("");
  ...
  if (context.elements.monthlyYearOneHeading) context.elements.monthlyYearOneHeading.textContent = String(displaySeries[0]?.year || currentYear - 2);
  if (context.elements.monthlyYearTwoHeading) context.elements.monthlyYearTwoHeading.textContent = String(displaySeries[1]?.year || currentYear - 1);
  if (context.elements.monthlyYearThreeHeading) context.elements.monthlyYearThreeHeading.textContent = String(displaySeries[2]?.year || currentYear);
  context.elements.monthlySalesBody.innerHTML = rows.join("");
  if (context.elements.monthlySalesFoot) {
    const totalCells = yearlyTotals
      .map((value) => `<td class="admin-table-number${numberStateClass(value)}">${escapeHtml(formatMoney(value))}</td>`)
      .join("");
...
export function resetReceivables(context) {
  if (context.elements.receivablesPanel) context.elements.receivablesPanel.hidden = false;
  if (context.elements.receivablesOpenValue) context.elements.receivablesOpenValue.textContent = "-";
...
export function renderReceivablesTable(context) {
  ...
  const totalPages = Math.max(1, Math.ceil(context.state.currentReceivables.length / RECEIVABLES_PAGE_SIZE));
  context.state.currentReceivablesPage = Math.min(context.state.currentReceivablesPage, totalPages);
  const start = (context.state.currentReceivablesPage - 1) * RECEIVABLES_PAGE_SIZE;
  const pageItems = context.state.currentReceivables.slice(start, start + RECEIVABLES_PAGE_SIZE);
  ...
  if (context.elements.receivablesPagination) context.elements.receivablesPagination.hidden = !context.state.currentReceivables.length;
  if (context.elements.receivablesPageInfo) {
    context.elements.receivablesPageInfo.textContent = context.state.currentReceivables.length
      ? `Σελίδα ${context.state.currentReceivablesPage} από ${totalPages}`
      : "Σελίδα 1 από 1";
  }
  if (context.elements.receivablesPrevBtn) context.elements.receivablesPrevBtn.disabled = context.state.currentReceivablesPage <= 1;
  if (context.elements.receivablesNextBtn) context.elements.receivablesNextBtn.disabled = context.state.currentReceivablesPage >= totalPages;
}

export function renderReceivables(context, receivables) {
  context.state.currentReceivables = Array.isArray(receivables?.items) ? receivables.items : [];
  context.state.currentReceivablesPage = 1;
  context.elements.receivablesOpenValue.textContent = formatMoney(receivables?.open_balance);
...
export function renderProductSales(context) {
  ...
  const sortedItems = filterProductItems(getSortedProductSalesForTable(context), context.state.currentProductSalesFilters);

  if (context.elements.productSalesMetricHeading) {
    context.elements.productSalesMetricHeading.textContent = metric === "pieces" ? "Τεμάχια" : "Τζίρος";
  }
  if (context.elements.productSalesSecondaryMetricHeading) {
    context.elements.productSalesSecondaryMetricHeading.textContent = secondaryMetric === "pieces" ? "Τεμάχια" : "Τζίρος";
  }

  const totalPages = Math.max(1, Math.ceil(sortedItems.length / PRODUCT_SALES_PAGE_SIZE));
  context.state.currentProductSalesPage = Math.min(context.state.currentProductSalesPage, totalPages);
  const start = (context.state.currentProductSalesPage - 1) * PRODUCT_SALES_PAGE_SIZE;
  ...
  context.elements.productSalesBody.innerHTML = pageItems.length
    ? pageItems
        .map((item) => {
          ...
          const secondaryMetricValue = secondaryMetric === "pieces" ? formatNumber(item.pieces) : formatMoney(item.revenue);
          return `
            <tr>
              <td>${escapeHtml(item.code)}</td>
  ...
  if (context.elements.productSalesPagination) context.elements.productSalesPagination.hidden = !sortedItems.length;
  ...
  if (context.elements.productSalesPrevBtn) context.elements.productSalesPrevBtn.disabled = context.state.currentProductSalesPage <= 1;
  if (context.elements.productSalesNextBtn) context.elements.productSalesNextBtn.disabled = context.state.currentProductSalesPage >= totalPages;
  updateSortIndicators("product-sales", context.state.productSalesSort);
}
...
export function renderRecentOrdersTable(context) {
  ...
  context.state.currentRecentOrdersPage = Math.min(context.state.currentRecentOrdersPage, totalPages);
  const start = (context.state.currentRecentOrdersPage - 1) * RECENT_ORDERS_PAGE_SIZE;
  ...
  context.elements.recentOrdersBody.innerHTML = pageItems.length
    ? pageItems
        .map((item) => {
          const isActive = String(context.state.selectedOrderId || "") === String(item.order_id || "");
          return `
            <tr>
  ...
  if (context.elements.recentOrdersPagination) context.elements.recentOrdersPagination.hidden = !recentOrders.length;
  ...
  if (context.elements.recentOrdersPrevBtn) context.elements.recentOrdersPrevBtn.disabled = context.state.currentRecentOrdersPage <= 1;
  if (context.elements.recentOrdersNextBtn) context.elements.recentOrdersNextBtn.disabled = context.state.currentRecentOrdersPage >= totalPages;
...
export function renderPreApprovalOrdersTable(context) {
  ...
  context.state.currentPreApprovalOrdersPage = Math.min(context.state.currentPreApprovalOrdersPage, totalPages);
  const start = (context.state.currentPreApprovalOrdersPage - 1) * PRE_APPROVAL_ORDERS_PAGE_SIZE;
  ...
  if (context.elements.preApprovalOrdersPagination) context.elements.preApprovalOrdersPagination.hidden = !preApprovalOrders.length;
  if (context.elements.preApprovalOrdersPageInfo) {
    context.elements.preApprovalOrdersPageInfo.textContent = preApprovalOrders.length
      ? `Σελίδα ${context.state.currentPreApprovalOrdersPage} από ${totalPages}`
      : "Σελίδα 1 από 1";
  }
  if (context.elements.preApprovalOrdersPrevBtn) context.elements.preApprovalOrdersPrevBtn.disabled = context.state.currentPreApprovalOrdersPage <= 1;
  if (context.elements.preApprovalOrdersNextBtn) context.elements.preApprovalOrdersNextBtn.disabled = context.state.currentPreApprovalOrdersPage >= totalPages;
}
...
export function renderOpenOrdersTable(context) {
  ...
  context.state.currentOpenOrdersPage = Math.min(context.state.currentOpenOrdersPage, totalPages);
  const start = (context.state.currentOpenOrdersPage - 1) * OPEN_ORDERS_PAGE_SIZE;
  ...
  if (context.elements.openOrdersPagination) context.elements.openOrdersPagination.hidden = !openOrders.length;
  ...
  if (context.elements.openOrdersPrevBtn) context.elements.openOrdersPrevBtn.disabled = context.state.currentOpenOrdersPage <= 1;
  if (context.elements.openOrdersNextBtn) context.elements.openOrdersNextBtn.disabled = context.state.currentOpenOrdersPage >= totalPages;
}
...
export function resetStats(context) {
  ...
  if (context.elements.totalOrdersValue) context.elements.totalOrdersValue.textContent = "0";
  context.elements.totalPiecesValue.textContent = "0";
  context.elements.totalRevenueValue.textContent = "-";
  if (context.elements.activeDocumentsValue) context.elements.activeDocumentsValue.textContent = "0";
  context.elements.averageOrderValue.textContent = "-";
  if (context.elements.daysSinceLastOrderValue) context.elements.daysSinceLastOrderValue.textContent = "-";
  ...
  if (context.elements.lastOrderDateValue) context.elements.lastOrderDateValue.textContent = "-";
  ...
  if (context.elements.recentOrdersPagination) context.elements.recentOrdersPagination.hidden = true;
  if (context.elements.recentOrdersPageInfo) context.elements.recentOrdersPageInfo.textContent = "Σελίδα 1 από 1";
  ...
  if (context.elements.openOrdersPagination) context.elements.openOrdersPagination.hidden = true;
  if (context.elements.openOrdersPageInfo) context.elements.openOrdersPageInfo.textContent = "Σελίδα 1 από 1";
  ...
  if (context.elements.preApprovalOrdersPagination) context.elements.preApprovalOrdersPagination.hidden = true;
  if (context.elements.preApprovalOrdersPageInfo) context.elements.preApprovalOrdersPageInfo.textContent = "Σελίδα 1 από 1";
  if (context.elements.openRankedOrderFormBtn) context.elements.openRankedOrderFormBtn.disabled = true;
  ...
  context.state.preApprovalOrdersSort = { key: "created_at", direction: "desc" };
  updateSortIndicators("recent", context.state.recentOrdersSort);
  updateSortIndicators("product-sales", context.state.productSalesSort);
  updateSortIndicators("open", context.state.openOrdersSort);
  updateSortIndicators("pre-approval", context.state.preApprovalOrdersSort);
...
export function renderStats(context, data) {
  ...
  context.state.currentProductSales = Array.isArray(productSales.items) ? productSales.items : [];
  context.state.currentTopProductsByQty = topProductsByQty;
  context.state.currentTopProductsByValue = topProductsByValue;
  if (context.elements.openRankedOrderFormBtn) context.elements.openRankedOrderFormBtn.disabled = context.state.currentProductSales.length === 0;
  context.state.selectedOrderId = null;
  context.state.currentProductSalesPage = 1;
  context.state.currentRecentOrdersPage = 1;
  context.state.currentOpenOrdersPage = 1;
  ...
  if (customer.branch_code) metaParts.push(`Υποκατάστημα: ${customer.branch_code}`);
  if (customer.branch_description) metaParts.push(customer.branch_description);
  if (customer.aggregation_level) metaParts.push(`Επίπεδο: ${context.formatAggregationLevelLabel(customer.aggregation_level)}`);
  if (customer.chain_name) metaParts.push(`Αλυσίδα: ${customer.chain_name}`);

  context.elements.customerNameHeading.textContent = customer.name || "Άγνωστος πελάτης";
  context.elements.customerMeta.textContent = metaParts.filter(Boolean).join(" | ") || "-";
  if (context.elements.totalOrdersValue) context.elements.totalOrdersValue.textContent = formatNumber(summary.total_orders ?? 0);
  if (context.elements.totalPiecesValue) context.elements.totalPiecesValue.textContent = formatNumber(summary.total_pieces ?? 0);
  if (context.elements.totalRevenueValue) context.elements.totalRevenueValue.textContent = formatMoney(summary.total_revenue);
  context.elements.averageOrderValue.textContent = formatMoney(summary.average_order_value);
  if (context.elements.daysSinceLastOrderValue) context.elements.daysSinceLastOrderValue.textContent = formatDays(summary.days_since_last_order);
  context.elements.averageDaysBetweenOrdersValue.textContent = formatDays(summary.average_days_between_orders);
  ...
  if (data?.range_summary) context.cacheRangeSummary(statsKey, context.state.currentSalesTimeRange, data.range_summary);
  const detailedOrdersForCards =
    context.state.allRangeDetailedOrders.length && context.state.allRangeStatsKey === statsKey
      ? context.state.allRangeDetailedOrders
      : context.state.currentDetailedOrders;
  const recentRange = context.getCardTimeRangeValue("recent_executed");
  if (!context.getCachedRangeSummary(statsKey, recentRange)) {
    void context.fetchRangeSummary(recentRange, customer.code, customer.branch_code || "", context.state.currentCustomerSearchFilters);
  }
  const recentRangeSummary = context.getCachedRangeSummary(statsKey, recentRange);
  const recentExecutedCount = recentRangeSummary
    ? Number(recentRangeSummary.total_orders || 0)
    : context.filterOrdersByRange(detailedOrdersForCards, recentRange, now).length;
  ...
  if (!context.getCachedRangeSummary(statsKey, piecesRange)) {
    void context.fetchRangeSummary(piecesRange, customer.code, customer.branch_code || "", context.state.currentCustomerSearchFilters);
  }
  if (!context.getCachedRangeSummary(statsKey, revenueRange)) {
    void context.fetchRangeSummary(revenueRange, customer.code, customer.branch_code || "", context.state.currentCustomerSearchFilters);
  }
  const piecesRangeSummary = context.getCachedRangeSummary(statsKey, piecesRange);
  const revenueRangeSummary = context.getCachedRangeSummary(statsKey, revenueRange);
  const ordersForPieces = context.filterOrdersByRange(detailedOrdersForCards, piecesRange, now);
  const ordersForRevenue = context.filterOrdersByRange(detailedOrdersForCards, revenueRange, now);
  ...
  const activeDocumentIds = new Set(
    [...preApprovalOrdersForTable, ...openOrdersForTable].map((order) => String(order?.order_id || "")).filter(Boolean),
  );

  if (context.elements.activeDocumentsValue) context.elements.activeDocumentsValue.textContent = formatNumber(activeDocumentIds.size);
  context.elements.acceptedOrdersValue.textContent = formatNumber(preApprovalCount);
  context.elements.inProgressOrdersValue.textContent = formatNumber(openCount);
  context.elements.invoicedOrdersValue.textContent = formatNumber(recentExecutedCount);
  if (context.elements.totalPiecesValue) context.elements.totalPiecesValue.textContent = formatNumber(piecesTotal);
  if (context.elements.totalRevenueValue) context.elements.totalRevenueValue.textContent = formatMoney(revenueTotal);
  if (context.elements.lastOrderDateValue) context.elements.lastOrderDateValue.textContent = formatDate(summary.last_order_date);

  renderBranchSelector(context, customer.code, availableBranches, customer.branch_code || "");
  renderMonthlySales(context, monthlySales);
  if (context.elements.receivablesPanel) context.elements.receivablesPanel.hidden = isBranchView;
  renderReceivables(context, receivables);
  context.elements.productSalesMetric.value = productSales.metric === "pieces" ? "pieces" : "revenue";
