...
export function getSortedProductSalesForTable(context) {
  ...
  return [...context.state.currentProductSales].sort((a, b) => {
    ...
    if (key === "code" || key === "description") {
      ...
      compare = compareSortableValues(
        a?.[primaryMetricField],
        b?.[primaryMetricField],
        {
          direction: sortState.direction,
          numeric: true,
        },
      ...
      compare = compareSortableValues(
        a?.[secondaryMetricField],
        b?.[secondaryMetricField],
        {
          direction: sortState.direction,
          numeric: true,
        },
...
export function matchesProductTableFilters(item, filters = {}) {
  ...
  if (descriptionFilter && !description.includes(descriptionFilter))
    return false;
  return true;
}
...
export function getRecentOrdersForTable(context) {
  ...
  return [...sourceOrders].sort((a, b) => {
    ...
    if (compare !== 0) return compare;
    return compareSortableValues(a?.created_at, b?.created_at, {
      direction: "desc",
      date: true,
    });
  });
...
export function getOpenOrdersForTable(context) {
  ...
  const cutoff = new Date(
    now.getTime() - OPEN_ORDERS_TIME_RANGE_DAYS * 86400000,
  );
...
export function getPreApprovalOrdersForTable(context) {
  const now = new Date();
  const cutoff = new Date(
    now.getTime() - PRE_APPROVAL_ORDERS_TIME_RANGE_DAYS * 86400000,
  );
