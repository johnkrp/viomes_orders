...
const IMPORT_HEALTH_DEFAULT_SUMMARY =
  "Δεν έχει φορτωθεί ακόμη κατάσταση import.";
...
const els = {
  ...
  importHealthStatusCard: document.getElementById("importHealthStatusCard"),
  importHealthStatus: document.getElementById("importHealthStatus"),
  importHealthDataset: document.getElementById("importHealthDataset"),
  importHealthFinishedAt: document.getElementById("importHealthFinishedAt"),
  importHealthRows: document.getElementById("importHealthRows"),
  importHealthSummary: document.getElementById("importHealthSummary"),
};
...
function resetImportHealth(message = IMPORT_HEALTH_DEFAULT_SUMMARY) {
  if (els.importHealthStatusCard) els.importHealthStatusCard.dataset.state = "idle";
  if (els.importHealthStatus) els.importHealthStatus.textContent = "-";
  if (els.importHealthDataset) els.importHealthDataset.textContent = "-";
  if (els.importHealthFinishedAt) els.importHealthFinishedAt.textContent = "-";
  if (els.importHealthRows) els.importHealthRows.textContent = "-";
  if (els.importHealthSummary) els.importHealthSummary.textContent = message;
}

function renderImportHealth(payload) {
  const latestRun = payload?.latest_import_run || null;
  const invariants = payload?.invariants || {};
  const unmappedTypes = Array.isArray(invariants.unmapped_document_types)
    ? invariants.unmapped_document_types.length
    : 0;
  const issues = [];

  if (Number(invariants.duplicate_groups || 0) > 0) {
    issues.push(`${formatNumber(invariants.duplicate_groups)} duplicate group(s)`);
  }
  if (Number(invariants.missing_mirrors || 0) > 0) {
    issues.push(`${formatNumber(invariants.missing_mirrors)} missing mirrored customer(s)`);
  }
  if (Number(invariants.orphan_mirrors || 0) > 0) {
    issues.push(`${formatNumber(invariants.orphan_mirrors)} orphan mirrored customer(s)`);
  }
  if (unmappedTypes > 0) {
    issues.push(`${formatNumber(unmappedTypes)} unmapped document type(s)`);
  }

  if (els.importHealthStatusCard) {
    els.importHealthStatusCard.dataset.state = payload?.ok ? "ok" : latestRun ? "warn" : "idle";
  }
  if (els.importHealthStatus) {
    els.importHealthStatus.textContent = payload?.ok
      ? "Healthy"
      : latestRun
        ? "Needs review"
        : "No data";
  }
  if (els.importHealthDataset) {
    els.importHealthDataset.textContent = latestRun?.dataset || "-";
  }
  if (els.importHealthFinishedAt) {
    els.importHealthFinishedAt.textContent = formatDateTime(
      latestRun?.finished_at || latestRun?.started_at,
    );
  }
  if (els.importHealthRows) {
    els.importHealthRows.textContent = latestRun
      ? `${formatNumber(latestRun.rows_upserted)} / ${formatNumber(latestRun.rows_in)}`
      : "-";
  }
  if (els.importHealthSummary) {
    els.importHealthSummary.textContent = issues.length
      ? issues.join(" | ")
      : latestRun
        ? `Latest run: ${latestRun.status || "unknown"}`
        : IMPORT_HEALTH_DEFAULT_SUMMARY;
  }
}

async function loadImportHealth() {
  if (!els.dashboardPanel || els.dashboardPanel.hidden) {
    resetImportHealth();
    return;
  }
  ...
  try {
    ...
    renderImportHealth(payload);
  } catch (error) {
    resetImportHealth(`Import health unavailable: ${error.message}`);
  }
}
...
function applySearchLoadingState(isLoading, message = "Ξ¦ΟΟΟ„Ο‰ΟƒΞ·...") {
  ...
  if (els.searchCustomersBtn) {
    ...
    els.searchCustomersBtn.textContent = busy ? "Ξ‘Ξ½Ξ±Ξ¶Ξ®Ο„Ξ·ΟƒΞ·..." : "Ξ‘Ξ½Ξ±Ξ¶Ξ®Ο„Ξ·ΟƒΞ·";
  }
...
async function setSearchLoading(isLoading, message = "Ξ¦ΟΟΟ„Ο‰ΟƒΞ·...") {
...
function applyStatsLoadingState(isLoading, message = "Ξ¦ΟΟΟ„Ο‰ΟƒΞ· ΟƒΟ„ΞΏΞΉΟ‡ΞµΞ―Ο‰Ξ½ Ο€ΞµΞ»Ξ¬Ο„Ξ·...") {
  const busy = Boolean(isLoading);
...
async function setStatsLoading(isLoading, message = "Ξ¦ΟΟΟ„Ο‰ΟƒΞ· ΟƒΟ„ΞΏΞΉΟ‡ΞµΞ―Ο‰Ξ½ Ο€ΞµΞ»Ξ¬Ο„Ξ·...") {
...
function setAuthenticatedUI(me) {
  ...
  if (!authenticated) {
    resetImportHealth();
    els.sessionInfo.textContent = "Ξ”ΞµΞ½ Ο…Ο€Ξ¬ΟΟ‡ΞµΞΉ ΞµΞ½ΞµΟΞ³Ξ® ΟƒΟ…Ξ½ΞµΞ΄ΟΞ―Ξ± admin.";
    return;
  }

  els.sessionInfo.textContent = `Ξ£Ο…Ξ½Ξ΄ΞµΞ΄ΞµΞΌΞ­Ξ½ΞΏΟ‚ Ο‰Ο‚ ${me.username}. API: ${API_BASE || "same-origin"}`;
}
...
function resetProductSales() {
  ...
  if (els.productSalesMetricHeading) els.productSalesMetricHeading.textContent = "Ξ¤Ξ¶Ξ―ΟΞΏΟ‚";
  if (els.productSalesPagination) els.productSalesPagination.hidden = true;
  if (els.productSalesPageInfo) els.productSalesPageInfo.textContent = "Ξ£ΞµΞ»Ξ―Ξ΄Ξ± 1 Ξ±Ο€Ο 1";
  ...
  if (els.productSalesBody) {
    els.productSalesBody.innerHTML = `
      <tr>
        <td colspan="6" class="admin-table-empty">Ξ”ΞµΞ½ Ξ­Ο‡ΞΏΟ…Ξ½ Ο†ΞΏΟΟ„Ο‰ΞΈΞµΞ― Ξ±ΞΊΟΞΌΞ· Ο€Ο‰Ξ»Ξ®ΟƒΞµΞΉΟ‚ Ο€ΟΞΏΟΟΞ½Ο„Ο‰Ξ½.</td>
      </tr>
    `;
  }
}
...
function resetSearchResults() {
  ...
  if (els.searchResultsBody) {
    els.searchResultsBody.innerHTML = `
      <tr>
        <td colspan="5" class="admin-table-empty">Ξ”ΞµΞ½ Ο…Ο€Ξ¬ΟΟ‡ΞΏΟ…Ξ½ Ξ±ΞΊΟΞΌΞ· Ξ±Ο€ΞΏΟ„ΞµΞ»Ξ­ΟƒΞΌΞ±Ο„Ξ±.</td>
      </tr>
    `;
  }
}
...
function resetBranchSelector() {
  ...
  if (els.branchSelector) {
    els.branchSelector.innerHTML = `<option value="">ΞΞ»Ξ± Ο„Ξ± Ο…Ο€ΞΏΞΊΞ±Ο„Ξ±ΟƒΟ„Ξ®ΞΌΞ±Ο„Ξ±</option>`;
...
function getBranchOptionLabel(branch) {
  ...
  return [code, description].filter(Boolean).join(" | ") || "Ξ§Ο‰ΟΞ―Ο‚ ΟƒΟ„ΞΏΞΉΟ‡ΞµΞ―Ξ± Ο…Ο€ΞΏΞΊΞ±Ο„Ξ±ΟƒΟ„Ξ®ΞΌΞ±Ο„ΞΏΟ‚";
}
...
function renderFilteredBranchOptions(branches, selectedBranchCode = "") {
  ...
  els.branchSelector.innerHTML = [
    `<option value="">ΞΞ»Ξ± Ο„Ξ± Ο…Ο€ΞΏΞΊΞ±Ο„Ξ±ΟƒΟ„Ξ®ΞΌΞ±Ο„Ξ±</option>`,
    ...items.map((branch) => {
      const code = branch.branch_code || "";
...
async function performCustomerSearch(filters, options = {}) {
  ...
  if (!silent) {
    setStatus(`Ξ’ΟΞ­ΞΈΞ·ΞΊΞ±Ξ½ ${payload.total} Ξ±Ο€ΞΏΟ„ΞµΞ»Ξ­ΟƒΞΌΞ±Ο„Ξ±.`, "ok");
  }
  return payload;
}
...
function resetMonthlySales() {
  ...
  if (els.monthlySalesBody) {
    els.monthlySalesBody.innerHTML = `
      <tr>
        <td colspan="5" class="admin-table-empty">Ξ”ΞµΞ½ Ξ­Ο‡ΞΏΟ…Ξ½ Ο†ΞΏΟΟ„Ο‰ΞΈΞµΞ― Ξ±ΞΊΟΞΌΞ· ΞΌΞ·Ξ½ΞΉΞ±Ξ―ΞµΟ‚ Ο€Ο‰Ξ»Ξ®ΟƒΞµΞΉΟ‚.</td>
      </tr>
    `;
  }
...
function resetStats() {
  ...
  els.customerNameHeading.textContent = "Ξ ΞµΞ»Ξ¬Ο„Ξ·Ο‚";
  ...
  els.topProductsQtyBody.innerHTML = `
    <tr>
      <td colspan="5" class="admin-table-empty">Ξ”ΞµΞ½ Ο…Ο€Ξ¬ΟΟ‡ΞΏΟ…Ξ½ Ξ±ΞΊΟΞΌΞ· Ξ΄ΞµΞ΄ΞΏΞΌΞ­Ξ½Ξ±.</td>
    </tr>
  `;
  ...
  if (recentOrdersHeadRow) {
    recentOrdersHeadRow.innerHTML = `
      ...
      <th>Order Date</th>
      <th>Sent Date</th>
      <th class="admin-table-number">Lines</th>
      <th class="admin-table-number">Pieces</th>
      <th class="admin-table-number">Value</th>
      <th class="admin-table-number">Avg Discount</th>
      <th>Action</th>
    `;
  }
  els.recentOrdersBody.innerHTML = `
    <tr>
      <td colspan="8" class="admin-table-empty">Ξ”ΞµΞ½ Ο…Ο€Ξ¬ΟΟ‡ΞΏΟ…Ξ½ Ξ±ΞΊΟΞΌΞ· Ξ΄ΞµΞ΄ΞΏΞΌΞ­Ξ½Ξ±.</td>
    </tr>
  `;
  ...
  els.detailedOrdersList.innerHTML = `
    <article class="admin-order-card admin-order-empty">
      Ξ•Ο€ΞΉΞ»Ξ­ΞΎΟ„Ξµ ΞΌΞΉΞ± Ο€Ξ±ΟΞ±Ξ³Ξ³ΞµΞ»Ξ―Ξ± Ξ±Ο€Ο Ο„ΞΏΞ½ Ο€Ξ―Ξ½Ξ±ΞΊΞ± Ξ³ΞΉΞ± Ξ½Ξ± Ξ΄ΞµΞ―Ο„Ξµ Ο„ΞΉΟ‚ Ξ±Ξ½Ξ±Ξ»Ο…Ο„ΞΉΞΊΞ­Ο‚ Ξ³ΟΞ±ΞΌΞΌΞ­Ο‚.
    </article>
  `;
...
function formatAggregationLevelLabel(level) {
  if (level === "branch") return "Ο…Ο€ΞΏΞΊΞ±Ο„Ξ¬ΟƒΟ„Ξ·ΞΌΞ±";
  if (level === "customer") return "Ο€ΞµΞ»Ξ¬Ο„Ξ·Ο‚";
  return level || "";
}
...
function renderSearchResults(items, filters = {}) {
  ...
  els.searchResultsBody.innerHTML = currentSearchResults.length
    ...
    : `
        <tr>
          <td colspan="5" class="admin-table-empty">Ξ”ΞµΞ½ Ξ²ΟΞ­ΞΈΞ·ΞΊΞ±Ξ½ Ο€ΞµΞ»Ξ¬Ο„ΞµΟ‚${activeFilters ? ` Ξ³ΞΉΞ± "${escapeHtml(activeFilters)}"` : ""}.</td>
        </tr>
      `;
}
...
function renderSelectedOrderDetails() {
  ...
  const linesHtml = Array.isArray(selectedOrder.lines) && selectedOrder.lines.length
    ...
    : `
        <tr>
          <td colspan="6" class="admin-table-empty">Ξ”ΞµΞ½ Ο…Ο€Ξ¬ΟΟ‡ΞΏΟ…Ξ½ Ξ³ΟΞ±ΞΌΞΌΞ­Ο‚ Ο€Ξ±ΟΞ±Ξ³Ξ³ΞµΞ»Ξ―Ξ±Ο‚.</td>
        </tr>
      `;
  ...
  els.detailedOrdersList.innerHTML = `
    <article class="admin-order-card admin-order-card-active">
      <div class="admin-order-head">
        <div>
          <h3>Ξ Ξ±ΟΞ±Ξ³Ξ³ΞµΞ»Ξ―Ξ± #${escapeHtml(selectedOrder.order_id)}</h3>
          <p>Order: ${escapeHtml(formatDateTime(selectedOrder.ordered_at || selectedOrder.created_at))}</p>
        </div>
        <div class="admin-order-kpis">
          <span>${escapeHtml(formatNumber(selectedOrder.total_lines))} Ξ³ΟΞ±ΞΌΞΌΞ­Ο‚</span>
          <span>${escapeHtml(formatNumber(selectedOrder.total_pieces))} Ο„ΞµΞΌ.</span>
          <strong>${escapeHtml(formatMoney(selectedOrder.total_net_value))}</strong>
        </div>
      </div>
      <div class="admin-order-note">${escapeHtml(selectedOrder.notes || "Ξ§Ο‰ΟΞ―Ο‚ ΟƒΞ·ΞΌΞµΞΉΟΟƒΞµΞΉΟ‚")}</div>
      <div class="admin-order-meta">
        <span>Sent: ${escapeHtml(formatDateTime(selectedOrder.sent_at))}</span>
        <span>Ξ. Ξ­ΞΊΟ€Ο„Ο‰ΟƒΞ·: ${escapeHtml(formatPercentRoundedUp(selectedOrder.average_discount_pct))}</span>
      </div>
...
function renderProductSales() {
  ...
  if (els.productSalesMetricHeading) {
    els.productSalesMetricHeading.textContent = metric === "pieces" ? "Ξ¤ΞµΞΌΞ¬Ο‡ΞΉΞ±" : "Ξ¤Ξ¶Ξ―ΟΞΏΟ‚";
  }
  ...
  els.productSalesBody.innerHTML = pageItems.length
    ...
    : `
        <tr>
          <td colspan="7" class="admin-table-empty">Το τρέχον daily ledger export δεν περιέχει αναλυτικά ανοικτά υπόλοιπα ανά παραστατικό.</td>
        </tr>
      `;
  ...
  if (els.productSalesPageInfo) {
    els.productSalesPageInfo.textContent = sortedItems.length
      ? `Ξ£ΞµΞ»Ξ―Ξ΄Ξ± ${currentProductSalesPage} Ξ±Ο€Ο ${totalPages}`
      : "Ξ£ΞµΞ»Ξ―Ξ΄Ξ± 1 Ξ±Ο€Ο 1";
  }
...
function renderStats(data) {
  ...
  if (customer.branch_code) {
    metaParts.push(`Ο…Ο€ΞΏΞΊ.: ${customer.branch_code}`);
  }
  if (customer.branch_description) {
    metaParts.push(customer.branch_description);
  }
  if (customer.aggregation_level) {
    metaParts.push(`ΞµΟ€Ξ―Ο€ΞµΞ΄ΞΏ: ${formatAggregationLevelLabel(customer.aggregation_level)}`);
  }
  if (customer.chain_name) {
    metaParts.push(`Ξ±Ξ»Ο…ΟƒΞ―Ξ΄Ξ±: ${customer.chain_name}`);
  }

  els.customerNameHeading.textContent = customer.name || "Ξ†Ξ³Ξ½Ο‰ΟƒΟ„ΞΏΟ‚ Ο€ΞµΞ»Ξ¬Ο„Ξ·Ο‚";
  ...
  els.topProductsQtyBody.innerHTML = topProductsByQty.length
    ...
    : `
        <tr>
          <td colspan="5" class="admin-table-empty">Ξ”ΞµΞ½ Ξ²ΟΞ­ΞΈΞ·ΞΊΞ±Ξ½ top Ο€ΟΞΏΟΟΞ½Ο„Ξ± ΟƒΞµ Ο„ΞµΞΌΞ¬Ο‡ΞΉΞ±.</td>
        </tr>
      `;
  ...
  els.topProductsValueBody.innerHTML = topProductsByValue.length
    ...
    : `
        <tr>
          <td colspan="5" class="admin-table-empty">Ξ”ΞµΞ½ Ξ²ΟΞ­ΞΈΞ·ΞΊΞ±Ξ½ top Ο€ΟΞΏΟΟΞ½Ο„Ξ± ΟƒΞµ Ξ±ΞΎΞ―Ξ±.</td>
        </tr>
      `;
  ...
  els.recentOrdersBody.innerHTML = recentOrders.length
    ...
    : `
        <tr>
          <td colspan="8" class="admin-table-empty">Ξ”ΞµΞ½ Ξ²ΟΞ­ΞΈΞ·ΞΊΞ±Ξ½ Ο€ΟΟΟƒΟ†Ξ±Ο„ΞµΟ‚ Ο€Ξ±ΟΞ±Ξ³Ξ³ΞµΞ»Ξ―ΞµΟ‚.</td>
        </tr>
      `;
...
async function refreshSession(options = {}) {
  try {
    ...
    if (!me.authenticated && !options.silent) {
      setStatus("Ξ£Ο…Ξ½Ξ΄ΞµΞΈΞµΞ―Ο„Ξµ Ξ³ΞΉΞ± Ξ½Ξ± Ξ΄ΞµΞ―Ο„Ξµ ΟƒΟ„Ξ±Ο„ΞΉΟƒΟ„ΞΉΞΊΞ¬ Ο€ΞµΞ»Ξ±Ο„ΟΞ½.", "info");
    }
    ...
    if (!options.silent) {
      setStatus(`Ξ‘Ο€ΞΏΟ„Ο…Ο‡Ξ―Ξ± ΟƒΟΞ½Ξ΄ΞµΟƒΞ·Ο‚ ΞΌΞµ Ο„ΞΏ backend: ${error.message}`, "error");
    }
...
async function handleLogin(event) {
  ...
  if (!username || !password) {
    setStatus("Ξ£Ο…ΞΌΟ€Ξ»Ξ·ΟΟΟƒΟ„Ξµ username ΞΊΞ±ΞΉ password.", "error");
    return;
  }
  ...
  try {
    ...
    setStatus("Ξ— ΟƒΟΞ½Ξ΄ΞµΟƒΞ· ΞΏΞ»ΞΏΞΊΞ»Ξ·ΟΟΞΈΞ·ΞΊΞµ.", "ok");
  } catch (error) {
    setStatus(`Ξ— ΟƒΟΞ½Ξ΄ΞµΟƒΞ· Ξ±Ο€Ξ­Ο„Ο…Ο‡Ξµ: ${error.message}`, "error");
  } finally {
...
async function handleLogout() {
  ...
  try {
    ...
    setSearchPanelCollapsed(false);
    els.username.focus();
    setStatus("Ξ— Ξ±Ο€ΞΏΟƒΟΞ½Ξ΄ΞµΟƒΞ· ΞΏΞ»ΞΏΞΊΞ»Ξ·ΟΟΞΈΞ·ΞΊΞµ.", "ok");
  } catch (error) {
    setStatus(`Ξ— Ξ±Ο€ΞΏΟƒΟΞ½Ξ΄ΞµΟƒΞ· Ξ±Ο€Ξ­Ο„Ο…Ο‡Ξµ: ${error.message}`, "error");
  } finally {
...
async function fetchCustomerStats(customerCode, branchCode = "", scopeFilters = currentCustomerSearchFilters) {
  ...
  if (!customerCode) {
    setStatus("Ξ”ΟΟƒΟ„Ξµ ΞΊΟ‰Ξ΄ΞΉΞΊΟ Ο€ΞµΞ»Ξ¬Ο„Ξ·.", "error");
  ...
  const loadingMessage = branchCode
    ? `Ξ¦ΟΟΟ„Ο‰ΟƒΞ· ΟƒΟ„ΞΏΞΉΟ‡ΞµΞ―Ο‰Ξ½ Ξ³ΞΉΞ± Ο…Ο€ΞΏΞΊΞ±Ο„Ξ¬ΟƒΟ„Ξ·ΞΌΞ± ${branchCode}...`
    : "Ξ¦ΟΟΟ„Ο‰ΟƒΞ· ΟƒΟ„ΞΏΞΉΟ‡ΞµΞ―Ο‰Ξ½ Ο€ΞµΞ»Ξ¬Ο„Ξ·...";
  ...
  try {
    ...
    resetSearchSuggestions();
    setStatus(
      `Ξ¦ΞΏΟΟ„ΟΞΈΞ·ΞΊΞ±Ξ½ Ο„Ξ± ΟƒΟ„Ξ±Ο„ΞΉΟƒΟ„ΞΉΞΊΞ¬ Ξ³ΞΉΞ± Ο„ΞΏΞ½ ΞΊΟ‰Ξ΄ΞΉΞΊΟ ${customerCode}${currentBranchCode ? ` / Ο…Ο€ΞΏΞΊ. ${currentBranchCode}` : ""}.`,
      "ok",
    );
  } catch (error) {
    if (requestId !== currentStatsRequestId) return;
    resetStats();
    setStatus(`Ξ— Ο†ΟΟΟ„Ο‰ΟƒΞ· ΟƒΟ„Ξ±Ο„ΞΉΟƒΟ„ΞΉΞΊΟΞ½ Ξ±Ο€Ξ­Ο„Ο…Ο‡Ξµ: ${error.message}`, "error");
  } finally {
...
async function searchCustomers(event) {
  ...
  if (!hasCustomerSearchFilters(filters)) {
    setStatus("Ξ£Ο…ΞΌΟ€Ξ»Ξ·ΟΟΟƒΟ„Ξµ Ο„ΞΏΟ…Ξ»Ξ¬Ο‡ΞΉΟƒΟ„ΞΏΞ½ Ξ­Ξ½Ξ± Ο€ΞµΞ΄Ξ―ΞΏ Ξ±Ξ½Ξ±Ξ¶Ξ®Ο„Ξ·ΟƒΞ·Ο‚.", "error");
    els.customerNameQuery.focus();
    return;
  }

  const requestId = ++currentSearchRequestId;
  setSearchLoading(true, "Ξ‘Ξ½Ξ±Ξ¶Ξ®Ο„Ξ·ΟƒΞ· Ο€ΞµΞ»Ξ±Ο„ΟΞ½...");
  setStatus("Ξ‘Ξ½Ξ±Ξ¶Ξ®Ο„Ξ·ΟƒΞ· Ο€ΞµΞ»Ξ±Ο„ΟΞ½...", "info");
  try {
    ...
    if (requestId !== currentSearchRequestId) return;
    resetSearchResults();
    setStatus(`Ξ— Ξ±Ξ½Ξ±Ξ¶Ξ®Ο„Ξ·ΟƒΞ· Ο€ΞµΞ»Ξ¬Ο„Ξ· Ξ±Ο€Ξ­Ο„Ο…Ο‡Ξµ: ${error.message}`, "error");
  } finally {
